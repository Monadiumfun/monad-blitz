  ...
  return (
    <div className="fixed bottom-3 left-3 z-40 flex flex-col gap-1 pointer-events-none">
      {txs.map(tx => (
        <div
          key={tx.id}
          className={`flex items-center gap-2 rounded-lg border border-[#2a2a3a] bg-[#12121acc] px-2.5 py-1.5 text-[11px] backdrop-blur-sm pointer-events-auto ${
            tx.leaving ? 'animate-toast-out' : 'animate-fade-in'
          }`}
        >
          {tx.status === 'pending' && <span className="loader-ring" aria-label="loading" />}

          {tx.status === 'confirmed' && (
            <>
              <span className="w-1.5 h-1.5 rounded-full shrink-0 bg-[#6E54FF]" />
              {tx.txHash && (
                <a
          ...
          {tx.status === 'failed' && (
            <>
              <span className="w-1.5 h-1.5 rounded-full shrink-0 bg-[#e74c3c]" />
